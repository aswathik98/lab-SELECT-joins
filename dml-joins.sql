select * from role inner join user_new on role.id=user_new.id ORDER BY user_new.username ASC,role.name ASC;

select * from role right join  user_new on role.id = user_new.id order by user_new.username ASC ,role.name ASC;
